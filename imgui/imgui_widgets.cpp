...
// Highlightable text
void ImGui::HighlightText( const char* buffer, size_t bufferSize, const ImGuiID& id )
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    // const ImGuiID id = window->GetID( "ConsoleOutput2" );
    const ImVec2 frame_size = CalcItemSize( ImVec2(10, 10), CalcItemWidth(), g.FontSize * 8.0f + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    PushStyleVar(ImGuiStyleVar_ChildBorderSize, style.FrameBorderSize);
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0)); // Ensure no clip rect so mouse hover can reach FramePadding edges
    bool child_visible = BeginChildEx("", id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove);
    PopStyleVar(3);
    PopStyleColor();
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        // Take a copy of the initial buffer value (both in original UTF-8 format and converted to wchar)
        // From the moment we focused we are ignoring the content of 'buf' (unless we are in read-only mode)
        // state->InitialTextA.resize(bufferSize + 1);    // UTF-8. we use +1 to make sure that .Data is always pointing to at least an empty string.
        
        // Start edition
        const char* buf_end = NULL;
        state->TextW.resize(bufferSize + 1);          // wchar count <= UTF-8 count. we use +1 to make sure that .Data is always pointing to at least an empty string.
        state->TextA.resize(0);
        state->TextAIsValid = false;                // TextA is not valid yet (we will display buf until then)
        state->CurLenW = ImTextStrFromUtf8(state->TextW.Data, bufferSize, buffer, NULL, &buf_end);
        state->CurLenA = (int)(buf_end - buffer);      // We can't get the result from ImStrncpy() above because it is not UTF-8 aware. Here we'll cut off malformed UTF-8.
    ...
    // When read-only we always use the live data passed to the function
    // FIXME-OPT: Because our selection/cursor code currently needs the wide text we need to convert it when active, which is not ideal :(
    if (state != NULL && (render_cursor || render_selection))
    {
        const char* buf_end = NULL;
        state->TextW.resize(bufferSize + 1);
    ...
    // Note that we only use this limit on single-line InputText(), so a pathologically large line on a InputTextMultiline() would still crash.
    const int buf_display_max_length = 2 * 1024 * 1024;
    const char* buf_display = buffer; //-V595
    const char* buf_display_end = NULL; // We have specialized paths below for setting the length
